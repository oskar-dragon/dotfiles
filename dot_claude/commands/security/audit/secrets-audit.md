...
- Project type detection: !`fd "(package\.json|go\.mod)" . -d 2 | head -3 || echo "No build files detected"`
- Codebase size: !`fd "\.(js|ts|jsx|tsx|go|env|json|yaml|yml|toml)" . | wc -l | tr -d ' '` files
...
rg -i "api[-_]?key\s*[:=]\s*['\"]?[a-zA-Z0-9]{20,}['\"]?" \
  ...
  --type config --type js --type ts --type go \
...
echo "  Total files scanned: $(fd "\.(js|ts|go|env|json|yaml)" . | wc -l | tr -d ' ')"
