...
description: Parallel technical explanations using 10 sub-agents for multi-angle analysis
---
...
- Code files: !`fd "\.(go|js|ts)$" . | wc -l | tr -d ' ' || echo "0"`
- Configuration files: !`fd "(package\.json|go\.mod)$" . -d 3 | head -5 || echo "No config files"`
...
- Technology indicators: !`rg "(import|require|use|include|from)" . --type go --type typescript | head -5 | cut -d: -f3 | sort -u | head -3 || echo "No imports detected"`
...
```bash
# Context discovery
rg "TARGET_PATTERN" -B 5 -A 5 --type go --type typescript
...
fd "*.{go,ts,js}" -x rg "TARGET" {} \;

# Dependency mapping
rg "(struct|class|interface|type).*TARGET" -A 10
fd "*.{go,ts,js}" -x rg "extends.*TARGET|implements.*TARGET" {} \;
```
...
fd . -t d -d 3 | head -15
rg "(service|controller|repository|model)" --type go -o | sort -u

# Integration point analysis
rg "(api|endpoint|route|handler)" --type go --type typescript -B 2 -A 2
...
- **Framework-Specific Variants**: [React vs other framework]
...
- **Language References**: [Go Tour, TypeScript Handbook]
...
```json
{
  ...
  "project_context": {
    "technology_stack": ["go", "typescript"],
