...
- Code files: !`fd -e ts -e js -e go -e . | wc -l | tr -d ' ' || echo "0"`
- Technology stack: !`fd "(package\.json|go\.mod)" --max-depth 2 | head -5 || echo "No tech files detected"`
...
- Detect naming inconsistencies: rg "class|interface|struct|type" --type-add 'code:\*.{ts,js,go}' -t code
...
- Apply framework-specific standards (Hono, React, Go, etc.)
