...
interface NamingRules {
  files: {
    components: "kekab-case"; // UserProfile.tsx
    utilities: "kebab-case"; // string-utils.ts
    constants: "SCREAMING_SNAKE"; // API_ENDPOINTS.ts
    types: "kebab-case"; // UserTypes.ts
  };
  ...
  for await (const entry of walk(directory)) {
    if (entry.isFile) {
      ...
      if (content !== standardContent) {
        await fs.writeFile(entry.path, standardContent);
      }
    }
  }
...
function loadConfig(): StandardConfig {
  return {
    app: {
      ...
      environment: (process.env.APP_ENV || "development") as any,
    },
    server: {
...
// Standardized Vitest test structure
...
// Standardized Playwright E2E test structure
