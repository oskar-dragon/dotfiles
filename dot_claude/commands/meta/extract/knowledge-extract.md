...
- Build system detection: !`fd "(package\.json|go\.mod)" . -d 3 | head -5 || echo "No build files detected"`
- Documentation exists: !`fd "(README|ARCHITECTURE|DESIGN|DOCS)" . -t f -d 2 | head -3 || echo "No docs found"`
- Code languages: !`fd "\.(go|ts|js)$" . | head -5 | sed 's/.*\.//' | sort -u | tr '\n' ' ' || echo "No code files"`
...
entity_count=$(rg "struct|class|interface|enum" --type go --type ts | wc -l | tr -d ' ')
echo "Domain entities discovered: $entity_count"

# Error handling patterns
error_count=$(rg "Error|Exception|Result|Option" --type go --type ts | wc -l | tr -d ' ')
...
3. **Establish Maintenance**: Set up documentation update processes
