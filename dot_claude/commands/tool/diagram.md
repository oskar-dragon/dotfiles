...
- Project files: !`fd -t f -e ts -e js -e go . | head -20`
- Main entry points: !`fd "^(main|index|app)\\.(ts|js|go)$" . | head -10`
- Package info: !`fd "^(package\\.json|go\\.mod)$" . | head -5`
...
- ELSE:
  - Scan project structure: !`fd -t f -e ts -e js -e go . | wc -l`
...
- Locate API layers: !`rg "@(Controller|Route|Api)" -t ts -t js`
...
- Save Mermaid diagram to `/docs/diagrams` directory in a project
