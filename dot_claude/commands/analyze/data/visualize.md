...
- Config files: !`fd "(docker-compose)" . -t f | head -5 || echo "No infrastructure configs"`
- Database models: !`rg "(struct|class|interface|type).*\{" --type-add 'code:*.{go,ts,js}' -t code | head -5 || echo "No models found"`
