...
- Technology stack: !`fd -e json -e toml -e xml -e txt . | rg "(go\.mod|package\.json)" | head -5 || echo "No technology files detected"`
...
`/investigate What's the best state management solution for our React?`

Investigation would:

- Analyze current state handling in codebase
- Research React state management patterns (2025)
- Compare options (Redux, MobX, Context API, URL)
