...
- Configuration files: !`fd "(package\.json|go\.mod)$" . -d 3 || echo "No config files"`
...
- Technology stack indicators: !`rg "(import|require|use|include|from)" . --type go--type typescript | head -10 | cut -d: -f3 | sort -u | head -5 || echo "No imports detected"`

## Your Task

**IMMEDIATELY DEPLOY 10 PARALLEL ANALYSIS AGENTS** for comprehensive technical analysis: **$ARGUMENTS**

Think deeply about comprehensive technical implementation while maximizing parallel execution.
...
fd . -t f -e json -e toml -e xml -e go -e ts -e js | head -20

# Architecture and patterns
rg "(class|interface|struct|enum|type|module)" . --type go --type typescript | head -10
...
```json
{
  ...
  "technology_stack": {
    "primary": "go|typescript|multiple",
  ...
  "research_findings": {
    ...
    "implementation": { "confidence": 0.9, "sources": 5, "examples": [] },
